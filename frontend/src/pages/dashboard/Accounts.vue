...
async function unbindAccount(account: AdAccountItem | undefined) { if (!account) return; try { await ElMessageBox.confirm('已有投放任务或实例引用时不会删除历史数据。确定解绑吗？', '确认解绑账号', { type: 'warning' }); await accountApi.delete(account.id); ElMessage.success('账号已解绑'); drawerVisible.value = false; await load() } catch { /* 用户取消或后端返回错误 */ } }
