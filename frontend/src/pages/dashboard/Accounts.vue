<template>
  <div class="page-container">
    ...
    <el-dialog v-model="addDialogVisible" title="添加 Meta 广告用户" width="620px" destroy-on-close>
      ...
      <div v-else class="success-state"><el-icon><Connection /></el-icon><h3>Meta 授权成功</h3><p>BM、广告账户和 Facebook Page 正在后台同步，稍后可在账户和模板页面使用。</p></div>
      <template #footer>
...
const accountIsDeployable = (account: AdAccountItem) => account.is_deployable === true
...
function accountAvailabilityLabel(account: AdAccountItem) { return account.is_deployable ? '可投放' : (account.availability_reason || '不可投放') }
function accountAvailabilityType(account: AdAccountItem): 'success'|'danger'|'warning'|'info' { return accountIsDeployable(account) ? 'success' : !account.account_status || account.availability_reason?.includes('同步') ? 'warning' : 'danger' }
...
async function reauthorizeAccount(account: AdAccountItem) {
  const meta = account.business_id ? metaAccounts.value.find(item => item.id === account.business_id) : null
  if (meta) return authorizeBusiness({ businessId: meta.id } as TreeNode)
