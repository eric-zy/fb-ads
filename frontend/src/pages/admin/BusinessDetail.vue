<template>
  <div class="page-container" v-loading="loading">
    ...
    <el-row :gutter="16" class="stat-row">
      ...
      <el-col :xs="12" :sm="6">
        <el-card shadow="never" class="stat-card">
          <div class="stat-value ok">{{ stats.system_active }}</div>
          <div class="stat-label">可投放</div>
        </el-card>
      </el-col>
      <el-col :xs="12" :sm="6">
        <el-card shadow="never" class="stat-card">
          <div class="stat-value warn">{{ stats.system_disabled }}</div>
          <div class="stat-label">系统停用</div>
        </el-card>
      </el-col>
      <el-col :xs="12" :sm="6">
        <el-card shadow="never" class="stat-card">
          <div class="stat-value danger">{{ stats.meta_abnormal }}</div>
          <div class="stat-label">Meta 异常</div>
        </el-card>
      </el-col>
    </el-row>
      ...
      <el-tabs v-model="activeTab">
        ...
        <el-tab-pane label="广告账户" name="accounts">
          <el-table :data="accounts" stripe style="width: 100%">
            ...
            <el-table-column label="Meta 状态" width="120">
            ...
            <el-table-column label="系统状态" width="120">
              <template #default="{ row }">
                <el-tag :type="row.system_status === 'ACTIVE' ? 'success' : 'danger'" effect="light" round size="small">
                  {{ row.system_status === 'ACTIVE' ? '可投放' : '已停用' }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column label="已消费" min-width="130">
    ...
    <el-dialog v-model="importVisible" title="导入广告账户" width="680px" destroy-on-close>
      <div class="import-tip">
        从 Meta 拉取 {{ detail?.name }} 下的账户，勾选后导入。
        <el-button link type="primary" :loading="fetching" @click="fetchFromMeta">重新拉取</el-button>
      </div>
      ...
      <el-table
        ...
        stripe
        style="width: 100%"
        @selection-change="onCandidatesChange"
        ref="candidateTableRef"
      >
        <el-table-column type="selection" width="46" />
        <el-table-column prop="name" label="账户名" min-width="160" />
        <el-table-column prop="id" label="Account ID" min-width="150" />
        <el-table-column prop="account_status" label="状态" width="110" />
        <el-table-column prop="currency" label="货币" width="90" />
        <el-table-column label="已存在" width="100">
          <template #default="{ row }">
            <el-tag v-if="row._existing" type="info" size="small" effect="plain">已在库中</el-tag>
            <span v-else class="sub-text">新账户</span>
          </template>
        </el-table-column>
      ...
      <template #footer>
        <span class="selected-tip">已选择：{{ selectedCandidates.length }}</span>
        <el-button @click="importVisible = false">取消</el-button>
        <el-button type="primary" :loading="importing" @click="submitImport">导入选中</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function statusLabel(row: MetaAccountItem | null) {
  if (!row) return '-'
  return { ACTIVE: '启用', DISABLED: '已禁用', ARCHIVED: '已归档' }[row.status] || row.status
}
function statusType(row: MetaAccountItem | null): 'success' | 'danger' | 'info' {
  if (!row) return 'info'
...
async function loadDetail() {
  loading.value = true
  try {
    const { data } = await metaAccountApi.detail(businessId.value)
    detail.value = data
    accounts.value = data.accounts || []
    stats.value = data.account_stats || stats.value
  } catch (e: any) {
...
async function loadLogs() {
  try {
    const { data } = await metaAccountApi.syncLogs(businessId.value, { limit: 30 })
    syncLogs.value = data
  } catch {
    syncLogs.value = []
  }
...
async function syncAccounts() {
  syncing.value = true
  try {
    const { data } = await metaAccountApi.syncAccounts(businessId.value)
    ElMessage.success(`同步任务已提交（${data.job_id.slice(0, 8)}…）`)
    setTimeout(async () => {
      await loadDetail()
      await loadLogs()
    }, 3000)
...
async function verifyConnection() {
  try {
    const { data } = await metaAccountApi.verifyConnection(businessId.value)
...
async function toggleStatus(row: AdAccountItem) {
  try {
    if (row.system_status === 'ACTIVE') await accountApi.freeze(row.id, '管理员停用')
    else await accountApi.unfreeze(row.id)
    await loadDetail()
  } catch (e: any) {
    // 错误已由 utils/request.ts 全局拦截器弹框提示
  }
...
async function fetchFromMeta() {
  fetching.value = true
  try {
    // 复用后端的 Meta 拉取能力（dev 模式会返回空列表并提示）
    const { data } = await request.get(
      `/api/v1/meta-accounts/${businessId.value}/ad-accounts/from-meta`
    )
    const existingIds = new Set(accounts.value.map((a) => a.account_id))
    candidates.value = (data.accounts || []).map((a: any) => ({
      id: a.id,
      name: a.name,
      account_status: a.account_status,
      currency: a.currency,
      _existing: existingIds.has(a.id),
    }))
    if (data.dev_mode) {
      ElMessage.warning('开发模式：未配置 FB 凭据，无法拉取真实账户列表')
    }
  } catch (e: any) {
    // 错误已由 utils/request.ts 全局拦截器弹框提示
  } finally {
...
function onCandidatesChange(rows: any[]) {
  selectedCandidates.value = rows
}

async function submitImport() {
  if (!selectedCandidates.value.length) {
    ElMessage.warning('请至少勾选一个账户')
    return
  }
  importing.value = true
  try {
    await request.post(`/api/v1/meta-accounts/${businessId.value}/ad-accounts/import`, {
      account_ids: selectedCandidates.value.map((a) => a.id),
    })
...
function goBack() {
  router.push({ name: 'AdminMetaAccounts' })
}
