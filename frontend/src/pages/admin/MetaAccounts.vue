  ...
  <div class="page-container">
    ...
    <el-card class="card-shadow" shadow="never">
      <el-table :data="list" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="凭据状态" width="210">
          <template #default="{ row }">
            <div class="cred-cell">
              <div class="cred-main">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑 BM' : '新增 BM'" width="540px" destroy-on-close>
      <el-form :model="form" label-width="120px">
        ...
        <el-form-item label="Access Token">
          <el-input v-model="form.access_token" type="textarea" :rows="2" :placeholder="isEdit ? '留空表示不更换 Token（Token 由「换 Token」维护）' : 'BM 访问令牌，加密存入凭据表'" show-password />
        </el-form-item>
        <el-form-item label="App ID"><el-input v-model="form.app_id" placeholder="可选" /></el-form-item>
        <el-form-item label="时区"><el-input v-model="form.timezone" placeholder="可选，同步可自动回填" /></el-form-item>
        <el-form-item label="货币"><el-input v-model="form.currency" placeholder="可选，如 USD" /></el-form-item>
    ...
    <el-dialog v-model="rotateVisible" :title="`更换 Token - ${rotateTarget?.name || ''}`" width="540px" destroy-on-close>
      ...
      <el-form :model="rotateForm" label-width="120px">
        ...
        <el-form-item label="类型"><el-select v-model="rotateForm.token_type" style="width: 100%"><el-option label="USER（用户令牌）" value="USER" /><el-option label="SYSTEM_USER（系统用户）" value="SYSTEM_USER" /><el-option label="PAGE（主页令牌）" value="PAGE" /></el-select></el-form-item>
      </el-form>
      <template #footer><el-button @click="rotateVisible = false">取消</el-button><el-button type="primary" :loading="saving" @click="submitRotate">确认更换</el-button></template>
    </el-dialog>
    ...
    <el-dialog v-model="logsVisible" :title="`同步日志 - ${logsTarget?.name || ''}`" width="760px" destroy-on-close>
      <el-table :data="syncLogs" v-loading="logsLoading" size="small" max-height="420">
        ...
        <el-table-column label="状态" width="140"><template #default="{ row }"><el-tag :type="logStatusType(row.status)" effect="plain" size="small">{{ row.status }}</el-tag></template></el-table-column>
        <el-table-column label="结果" width="120"><template #default="{ row }">{{ row.success_count }} / {{ row.total_count }}</template></el-table-column>
      ...
      <template #footer><el-button @click="openLogs(logsTarget!)">刷新</el-button><el-button type="primary" @click="logsVisible = false">关闭</el-button></template>
    </el-dialog>
  </div>
</template>
...
import { metaAccountApi, credentialApi, type MetaAccountItem, type SyncLogItem } from '@/api/admin'
...
function syncType(row: MetaAccountItem): 'success' | 'danger' | 'warning' | 'info' { if (row.sync_status === 'SUCCESS') return 'success'; if (row.sync_status === 'FAILED') return 'danger'; if (row.sync_status === 'SYNCING') return 'warning'; return 'info' }
function statusLabel(row: MetaAccountItem) { return ({ ACTIVE: '启用', DISABLED: '已禁用', ARCHIVED: '已归档' } as Record<string, string>)[row.status] || row.status }
function statusType(row: MetaAccountItem): 'success' | 'danger' | 'info' { if (row.status === 'ACTIVE') return 'success'; if (row.status === 'DISABLED') return 'danger'; return 'info' }
...
async function setDefault(row: MetaAccountItem) { try { await metaAccountApi.setDefault(row.id); ElMessage.success('已切换默认 BM'); await load() } catch {} }
async function syncAccounts(row: MetaAccountItem) { try { const { data } = await metaAccountApi.syncAccounts(row.id); ElMessage.success({ message: `同步任务已提交（${data.job_id.slice(0, 8)}…），稍后可在「日志」中查看结果`, duration: 4000 }); setTimeout(load, 3000) } catch {} }
async function openLogs(row: MetaAccountItem) { logsTarget.value = row; logsVisible.value = true; logsLoading.value = true; try { const { data } = await metaAccountApi.syncLogs(row.id, { limit: 30 }); syncLogs.value = data } catch { syncLogs.value = [] } finally { logsLoading.value = false } }
async function verifyConnection(row: MetaAccountItem) { try { const { data } = await metaAccountApi.verifyConnection(row.id); if (data.dev_mode) ElMessage.warning('开发模式：未配置 FB 凭据，未做真实校验'); else if (data.ok) ElMessage.success('连接正常，Business ID 校验通过'); else ElMessage.error('校验失败：' + (data.error || '未知错误')) } catch {} }
function openRotate(row: MetaAccountItem) { rotateTarget.value = row; rotateForm.value = { access_token: '', token_type: 'USER' }; rotateVisible.value = true }
async function submitRotate() { if (!rotateTarget.value || !rotateForm.value.access_token) { ElMessage.warning('请填写新的 Access Token'); return }; saving.value = true; try { await metaAccountApi.rotateToken(rotateTarget.value.id, { access_token: rotateForm.value.access_token, token_type: rotateForm.value.token_type }); ElMessage.success('Token 已更换'); rotateVisible.value = false; await load() } catch {} finally { saving.value = false } }
async function onMoreCommand(cmd: string, row: MetaAccountItem) { if (cmd === 'delete') { try { await ElMessageBox.confirm(`确认删除 BM「${row.name}」？其名下凭据会一并清理。`, '警告', { type: 'error' }) } catch { return }; try { await metaAccountApi.remove(row.id); ElMessage.success('已删除'); await load() } catch {}; return }; const label = cmd === 'disable' ? '禁用' : '归档'; try { await ElMessageBox.confirm(`确认${label} BM「${row.name}」？`, '提示', { type: 'warning' }) } catch { return }; try { if (cmd === 'disable') await metaAccountApi.disable(row.id); else await metaAccountApi.archive(row.id); ElMessage.success(`已${label}`); await load() } catch {} }
function goAccounts(row: MetaAccountItem) { router.push({ name: 'AdminAccounts', query: { business_id: row.id } }) }
function goCredentials(row?: MetaAccountItem) { router.push({ name: 'AdminCredentials', query: row ? { meta_account_id: row.id } : {} }) }
function goDetail(row: MetaAccountItem) { router.push({ name: 'AdminBusinessDetail', params: { id: row.id } }) }
onMounted(load)
</script>

<style scoped>
.head-actions { display: flex; align-items: center; gap: 8px; }
.cred-cell { display: flex; flex-direction: column; gap: 2px; }
.cred-main { display: flex; align-items: center; gap: 6px; }
.cred-mask { font-family: 'Courier New', Courier, monospace; font-size: 12px; color: #909399; }
.sync-time { font-size: 12px; color: #909399; margin-top: 2px; }
