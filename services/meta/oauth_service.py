...
class MetaOAuthService:
    ...
    def exchange_code(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 部分 Token 类型不支持长期交换，短期 Token 仍可正常使用。
            result = short
    ...
    def verify_permissions(self, access_token: str) -> list[str]:
        """确认用户实际授予了配置中的全部权限，拒绝“部分授权”静默入库。"""
        ...
        required = {scope.strip() for scope in settings.FB_OAUTH_SCOPES.split(",") if scope.strip()}
